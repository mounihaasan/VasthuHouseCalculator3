function calculate() {
    ...
    // Store all filterable values
    let all_values = ["aysh", "adym", "karch", "aym", "nak", "vrm", "tidhi", "dikpth", "amsa", "ygm", "karnm", "thvm", "kulm", "lagnam"];
    
    // If no checkboxes are selected, disable filtering
    ...
    for (let e = a; e <= b; e++) {
        for (let f = c; f <= d; f++) {
            ...
            if (conditions_met) {
                ...
                cell3.innerText = all_values.map(val => `${val}: ${values[val]}`).join(", ");
